#!/usr/bin/env bash
# SeisBench EQTransformer v3 — same recipe as v2, but with a tighter
# detection-label window to fix the long-tail mask overfit observed in
# v2 (det_per_trace_mean=1.50, det_end_mae=31 s, spurious second
# detection on background noise — see viz_models_rose trace_005489.png).
#
# Recipe identical to v2:
...
#
# v2 → v3 delta:
#   * DETECTION_FACTOR=0.7  (was 1.4)
#     RoSE local events have PS-diff ~25-30 s; factor=1.4 made the
#     labelled event box ~60-72 s long — well past the actual coda.
#     factor=0.7 yields a labelled box from P to S + ~18 s, matching
#     local-coda physics. Expected outcome:
#       - det_per_trace_mean: 1.50 → ~1.05
...
torchrun --standalone --nproc_per_node="${NPROC_PER_NODE:-4}" \
  ...
  --out-dir  "${OUT_DIR:-/home/u2140647/RoSE/checkpoints/seisbench_rose/eqt_v3}" \
  --epochs            "${EPOCHS:-30}" \
  --batch-size        "${BATCH:-64}" \
  --lr                "${LR:-1e-4}" \
